...
public class TextApi {
     ...
     * @param pkPath   用户私钥
     * @param textUrl 请求接口地址
     */
    public TextApi(String secretId, String pkPath, String textUrl) {
        if (null == textUrl) {
            textUrl = ConfigUtil.NET_WORK.TEXT_API_URI;
        }

        this.secretId = secretId;
        this.url = textUrl + secretId;
        this.privateKey = SignatureAndVerifyUtil.readPrivateKey(pkPath);

    }

    /**
     * @param textList text 文件
     * @return
     */
    public JSONObject doTextApi(ArrayList<Text> textList) {
        ...
        try {
            // 返回网络请求数据
            classificationResult = HttpConnectionUtil.uploadText(url, String.valueOf(timestamp), String.valueOf(nonce), signature, textList);
